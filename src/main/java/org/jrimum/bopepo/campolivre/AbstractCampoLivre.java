...
abstract class AbstractCampoLivre extends BlockOfFields implements CampoLivre {
		...
		try {
			...
			if (BancosSuportados.isSuportado(titulo.getContaBancaria().getBanco().getCodigoDeCompensacaoBACEN().getCodigoFormatado())) {
				...
				default:
					 ...
					 */
					throw new NotSupportedCampoLivreException("Não há implementações de campo livre para o banco " + titulo.getContaBancaria().getBanco().getCodigoDeCompensacaoBACEN().getCodigoFormatado() + " compatíveis com as "
	...
	protected final static void checkDigitoDoCodigoDaAgencia(Titulo titulo) {
		...
		Strings.checkNotBlank(titulo.getContaBancaria().getAgencia().getDigitoVerificador(),
				format("Dígito verificador [\"%s\"] da agência bancária não pode ser vazio!", titulo.getContaBancaria().getAgencia().getDigitoVerificador()));
		Strings.checkNotNumeric(titulo.getContaBancaria().getAgencia().getDigitoVerificador(),
				format("Nesse contexto o dígito verificador [\"%s\"] da agência bancária deve ser numérico!", titulo.getContaBancaria().getNumeroDaConta().getDigitoDaConta()));
	}
	...
	protected final static void checkDigitoDoCodigoDoNumeroDaConta(Titulo titulo) {
		...
		Strings.checkNotBlank(titulo.getContaBancaria().getNumeroDaConta().getDigitoDaConta(),
		...
		Strings.checkNotNumeric(titulo.getContaBancaria().getNumeroDaConta().getDigitoDaConta(),
	...
	protected final static void checkTamanhoDoNossoNumero(Titulo titulo, int length) {
		...
		checkTamanhoNossoNumero(titulo, length, format("Tamanho [%s] do nosso número [\"%s\"] diferente do esperado [%s]!", StringUtils.length(titulo.getNossoNumero()), titulo.getNossoNumero(), length));
	}
	...
	protected final static void checkTamanhoDigitoDoNossoNumero(Titulo titulo, int length) {
		...
		checkTamanhoDigitoDoNossoNumero(titulo, length,
				format("Tamanho [%s] do dígito do nosso número [\"%s\"] diferente do esperado [%s]!", StringUtils.length(titulo.getDigitoDoNossoNumero()), titulo.getDigitoDoNossoNumero(), length));
	}
